...
class TopoChart {
  ...
  update(option) {
    ...
    this.lineTextGroup.remove();
    this.nodes.remove();
    this.lineArrows.remove();
  ...
  initOption(option) {
    ...
    this.edges = option.data.edges;
    this.scale = 1;
    this.lineGroup = null;
    this.lineTextGroup = null;
    this.nodes = null;
    this.lineArrows = null;
    this.drilldown = null;
    this.initPosition();
    this.initLink();
    this.initNode();
  ...
  initPosition() {
    ...
    let points = this.getVertices(origin, Math.min(width, height) * 0.3, this.data.length);
  ...
  initDrilldown() {
    this.drilldown = this.svg.append('g')
      .attr('class', 'drilldown')
      .style('display', 'none')
      .on('click', () => {
        this.option.onNodeClick(this.selectNodeData)
      })

    this.drilldown.append('rect')
      .attr('width', 100)
      .attr('height', 40)
      .attr('rx', 5)
      .attr('ry', 5)

    this.drilldown.append('text')
      .text('Drilldown')
      .attr('dy', 24)
      .attr('dx', 25)
    ...
    this.lineTextGroup.on('click', () => {
      this.hideDrilldown();
    })
    this.lineGroup.on('click', () => {
      this.hideDrilldown();
    })
  ...
  showDrilldown(data) {
    ...
    this.drilldown.attr('transform', `translate(${d3.event.layerX - 50},${d3.event.layerY - 40})`)
      .style('display', 'block');
  ...
  //初始化图标
  initDefineSymbol() {
    let defs = this.defs = this.container.append('svg:defs');
  ...
  initNode() {
    ...
    this.nodes = this.container.selectAll(".node")
      ...
      .on('click', function (data) {
        if (data.drilldown) {
          self.showDrilldown(data)
        }
        else {
          self.option.onNodeClick(data)
        }
      })
  ...
  //画节点语言标识
  drawNodeTip() {
    let nodeCodes = this.nodes.filter(item => item.iconType == 'app')
      .append('g')
    ...
    nodeCodes
      .append('circle')
      .attr('r', d => fontSize / 2 * d.tip.length / 2 + 3)
  ...
  drawNodeSymbol() {
    //绘制节点
    this.nodes.filter(item => item.iconType == 'app')
      .append("circle")
      .attr("r", symbolSize / 2)
      .attr("fill", '#fff')
    ...
    this.nodes.filter(item => item.iconType == 'database')
      .append('use')
    ...
    this.nodes.filter(item => item.iconType == 'cloud')
      .append('use')
  ...
  drawNodeExtra() {
    //如果是应用的时候
    let nodeOthers = this.nodes.filter(item => item.iconType == 'app')
      .append("text")
      .attr("x", symbolSize / 2 + padding)
      .attr("y", -5)
      .attr('class', 'node-other')

    nodeOthers.append('tspan')
      .text(d => d.extra[0]);
  ...
  //画节点正中心数据
  drawNodeText() {
    this.nodes.filter(item => item.iconType == 'app').append("text")
      .text(function (d) {
        return d.text;//实例数显示
      })
      .attr('dy', fontSize / 2)
  ...
  getVertices(origin, r, n) {
    ...
    var tempAngle = 0;
    while (i < n) {
      tempAngle = (i * angle * Math.PI) / 180 - 90;
      points.push({
